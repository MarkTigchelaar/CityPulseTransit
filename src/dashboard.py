...
def load_fleet_status():
    try:
        return pd.read_sql("SELECT * FROM public_transit.live_fleet_status", engine)
    except: return pd.DataFrame()

def load_station_congestion():
    try:
        return pd.read_sql("SELECT * FROM public_transit.live_station_congestion", engine)
    except: return pd.DataFrame()
...
while True:
    with placeholder.container():
        ...
        # Load Fresh Data
        df_fleet = load_fleet_status()
        df_stations = load_station_congestion()
