...
with current_trains as (
    select
        ...
        t.route_id
    from {{ ref('int_trains__recent_state') }} tcs
    join {{ ref('trains') }} t 
...
train_topology_base as (
    ...
    join {{ ref('int_route__topology') }} topo
        on ct.route_id = topo.route_id
        and ct.recent_stop_sequence = topo.stop_sequence
),

train_offsets as (
    select segment_id, train_id, train_position 
...
from train_topology_base base
left join train_offsets ofs
    on base.segment_id = ofs.segment_id 
